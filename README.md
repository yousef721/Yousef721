<div align="center">

<!-- Animated Header -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:512BD4,100:7B2FBE&height=230&section=header&text=Yousef%20Abdullah&fontSize=52&fontColor=ffffff&fontAlignY=36&desc=Full-Stack%20.NET%20Developer%20%7C%20ASP.NET%20Core%20%7C%20Egypt&descAlignY=60&descSize=22&animation=fadeIn" width="100%" />

<br/>

<!-- Typing Animation -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&pause=1000&color=512BD4&center=true&vCenter=true&width=650&lines=Building+Scalable+Web+Applications;ASP.NET+Core+%7C+REST+APIs+%7C+SQL+Server;Clean+Architecture+Enthusiast;Open+to+Work+%26+Freelance+Opportunities" />

<br/><br/>

<!-- Badges -->
<a href="https://github.com/yousef721">
  <img src="https://komarev.com/ghpvc/?username=yousef721&style=for-the-badge&color=512BD4&label=PROFILE+VIEWS" />
</a>
&nbsp;
<a href="https://github.com/yousef721?tab=followers">
  <img src="https://img.shields.io/github/followers/yousef721?style=for-the-badge&color=7B2FBE&label=FOLLOWERS" />
</a>

</div>

---

## 🪪 About Me

```yaml
Name         : Yousef Abdullah
Role         : Full-Stack .NET Developer
Location     : Egypt 🇪🇬
Focus        : ASP.NET Core | REST APIs | SQL Server
Architecture : Clean Architecture & Design Patterns
Status       : Open to Work & Freelance
```

💡 I specialize in building scalable, maintainable, and production-ready web applications using modern .NET technologies.

I focus on:
- Writing clean, structured, maintainable code
- Building secure and optimized REST APIs
- Designing efficient relational databases
- Applying Clean Architecture principles

---

## 🛠️ Tech Stack

<div align="center">

### ⚙️ Backend
<img src="https://skillicons.dev/icons?i=cs,dotnet" />

### 🎨 Frontend
<img src="https://skillicons.dev/icons?i=html,css,js,bootstrap" />

### 🗄️ Database & Tools
<img src="https://skillicons.dev/icons?i=git,github,visualstudio,postman" />

<br/>

<img src="https://img.shields.io/badge/SQL_Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white" />

</div>

---

## 📊 GitHub Analytics

<div align="center">

<img width="360" height="180" src="https://github-readme-stats-sigma-five.vercel.app/api?username=yousef721&show_icons=true&hide_border=true&title_color=512BD4&icon_color=7B2FBE&text_color=333&count_private=true&include_all_commits=true&bg_color=fafafa" />
<img width="360" height="180" src="https://streak-stats.demolab.com/?user=yousef721&hide_border=true&ring=512BD4&fire=7B2FBE&currStreakLabel=512BD4&sideLabels=333&dates=888&background=fafafa" />
<img width="360" height="180" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=yousef721&layout=compact&hide_border=true&title_color=512BD4&text_color=333&langs_count=6&bg_color=fafafa" />

</div>

<br/>

---

## 📬 Let's Connect

<div align="center">

<a href="https://linkedin.com/in/yousef-abdullah-076ba0249">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
&nbsp;&nbsp;
<a href="https://github.com/yousef721">
  <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

</div>

<br/>

<!-- Animated Footer -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:7B2FBE,100:512BD4&height=120&section=footer&animation=fadeIn" width="100%" />
